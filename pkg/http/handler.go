...
// makeMultiplyHandler creates the handler logic
func makeMultiplyHandler(m *http.ServeMux, endpoints endpoint.Endpoints, options []http1.ServerOption) {
	m.Handle("/multiply", http1.NewServer(endpoints.MultiplyEndpoint, decodeMultiplyRequest, encodeMultiplyResponse, options...))
}

// decodeMultiplyRequest is a transport/http.DecodeRequestFunc that decodes a
// JSON-encoded request from the HTTP request body.
func decodeMultiplyRequest(_ context.Context, r *http.Request) (interface{}, error) {
	req := endpoint.MultiplyRequest{}
	err := json.NewDecoder(r.Body).Decode(&req)
	return req, err
}

// encodeMultiplyResponse is a transport/http.EncodeResponseFunc that encodes
// the response as JSON to the response writer
func encodeMultiplyResponse(ctx context.Context, w http.ResponseWriter, response interface{}) (err error) {
	if f, ok := response.(endpoint.Failure); ok && f.Failed() != nil {
		ErrorEncoder(ctx, f.Failed(), w)
		return nil
	}
